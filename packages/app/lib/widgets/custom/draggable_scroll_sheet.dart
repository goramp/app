...
class _DraggableScrollableSheetScrollPosition
  ...
  void goBallistic(double velocity) {
    ...
    final AnimationController ballisticController = AnimationController(
      ...
      upperBound: 1.0,
      lowerBound: extent.minExtent / extent.maxExtent,
      vsync: context.vsync,
      duration: const Duration(milliseconds: 200),
    );

    final double flingVelocity = velocity / extent.maxExtent;
    void _tick() {
      extent.currentExtent = ballisticController.value;
    }
    ...
    if (flingVelocity < 0.0) {
      ...
      final minPoint = extent.minExtent / extent.maxExtent;
      final mid = (1 - minPoint) / 2 + minPoint;
      ballisticController
          .fling(velocity: extent.currentExtent > mid ? 2.0 : -2.0)
          .whenCompleteOrCancel(
            ballisticController.dispose,
          );
    }
...
class DraggableScrollableActuator extends StatelessWidget {
  ...
  DraggableScrollableActuator({Key? key, required this.child})
      : super(key: key);
